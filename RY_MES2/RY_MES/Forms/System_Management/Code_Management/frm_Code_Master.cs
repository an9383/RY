...
{
    public partial class frm_Code_Master : RY_MES.frm_Base
    {
        public frm_Code_Master()
        {
            InitializeComponent();
        }

        public frm_Code_Master(object[] paramArray)
        {
            InitializeComponent();
        }
        ...
        private void Get_Data_Grid(ucGridControl grid)
        {
            ...
            try
            {
                ...
                string sMsg = _RYMES_DB.GET_DATA("SM_CM_CODE_MASTER_LOAD", ref dt);
                if (string.IsNullOrEmpty(sMsg))
                {
                    grid.DataSource = dt;
                    view.Link_Column("GROUP_CODE", "frm_Code_Detail");
                }
                else
                {
        ...
        private void SHOW_EDIT(GridView gridView, object sender)
        {
            ...
            {
                txt_GROUP_CODE.Text = "";
                txt_GROUP_CODE_NAME.Text = "";
                txt_GROUP_CODE_DESC.Text = "";
                txt_ORDERBY.Text = "";
                txt_DESC1.Text = "";
                txt_DESC2.Text = "";
            ...
            else
            {
                ...
                txt_GROUP_CODE.Text = dataRow["GROUP_CODE"].ToString();
                txt_GROUP_CODE_NAME.Text = dataRow["GROUP_CODE_NAME"].ToString();
                txt_GROUP_CODE_DESC.Text = dataRow["GROUP_CODE_DESC"].ToString();
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string sMsg = _RYMES_DB.SET_DATA("SM_CM_CODE_MASTER_MERGE");
                if (string.IsNullOrEmpty(sMsg))
                {
        ...
        private void btn_Delete_Click(object sender, EventArgs e)
        {
            ...
            if (DialogResult.OK == MessageBox.Show(btn.Text + "  " + txt_GROUP_CODE_NAME.Text + " ?", btn.Text, MessageBoxButtons.OKCancel, MessageBoxIcon.Question))
            {
                ...
                try
                {
                    ...
                    string sMsg = _RYMES_DB.SET_DATA("SM_CM_CODE_MASTER_DELETE");
                    if (string.IsNullOrEmpty(sMsg))
                    {
