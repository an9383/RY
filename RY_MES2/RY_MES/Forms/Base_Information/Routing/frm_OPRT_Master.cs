...
{
    public partial class frm_OPRT_Master : RY_MES.frm_Base
    {
        private string rt_id = null;

        public frm_OPRT_Master()
        {
            InitializeComponent();
        }

        public frm_OPRT_Master(params object[] paramArray)
        {
            InitializeComponent();
            rt_id = paramArray[0].ToString();
        }
        ...
        private void Frm_Load(object sender, EventArgs e)
        {
            ...
            ucGridView1.FindFilterText = rt_id;
        }
        ...
        private void Get_Data_Grid(ucGridControl grid)
        {
            ...
            try
            {
                ...
                string sMsg = _RYMES_DB.GET_DATA("BI_RO_OPRT_MASTER_LOAD", ref dt);
        ...
        private void SHOW_EDIT(GridView gridView, object sender)
        {
            ...
            if (menu.Caption == "Add")
            {
                le_RT_ID.EditValue = null;
                le_FA_ID.EditValue = null;
                le_OP_ID.EditValue = null;
                txt_RT_STEP.Text = "";
                txt_SN_YN.EditValue = "";
                txt_SEND_RESULT_YN.EditValue = "";
                txt_CO_WORK_YN.EditValue = "";
                txt_NOTES.Text = "";

                le_RT_ID.Enabled = true;
            ...
            else
            {
                ...
                txt_RT_STEP.Text = dataRow["RT_STEP"].ToString();
                txt_SN_YN.EditValue = dataRow["SN_YN"].ToString();
                txt_SEND_RESULT_YN.EditValue = dataRow["SEND_RESULT_YN"].ToString();
                txt_CO_WORK_YN.EditValue = dataRow["CO_WORK_YN"].ToString();
                txt_NOTES.Text = dataRow["NOTES"].ToString();

                le_RT_ID.Enabled = false;
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string sMsg = _RYMES_DB.SET_DATA("BI_RO_OPRT_MASTER_MERGE");
                if (string.IsNullOrEmpty(sMsg))
                {
        ...
        private void btn_Delete_Click(object sender, EventArgs e)
        {
            ...
            if (DialogResult.OK == MessageBox.Show(btn.Text + "  " + le_RT_ID.EditValue + " ?", btn.Text, MessageBoxButtons.OKCancel, MessageBoxIcon.Question))
            {
                try
                {
                    _RYMES_DB._DB_Parameters.Add("@p_RT_ID", le_RT_ID.EditValue);
                    _RYMES_DB._DB_Parameters.Add("@p_FA_ID", le_FA_ID.EditValue);
                    _RYMES_DB._DB_Parameters.Add("@p_OP_ID", le_OP_ID.EditValue);
                    _RYMES_DB._DB_Parameters.Add("@p_CREATE_USER", _Main._User_Info["USER_CODE"].ToString());

                    string sMsg = _RYMES_DB.SET_DATA("BI_RO_OPRT_MASTER_DELETE");
                    if (string.IsNullOrEmpty(sMsg))
                    {
