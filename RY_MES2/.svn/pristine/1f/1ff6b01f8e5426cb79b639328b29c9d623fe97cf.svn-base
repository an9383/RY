...
{
    public partial class frm_Panel_List_CSI : RY_MES.frm_Base
    {
        private string fa_id = "CSI";

        public frm_Panel_List_CSI()
        {
            InitializeComponent();
        }
        ...
        private void Get_Data_Grid(ucGridControl grid)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(sMsg))
                {
                    ...
                    view.Columns["WAFER_NO"].Caption = "Panel/Plate 번호";
                    view.Columns["WAFER_STATUS"].Caption = "Panel/Plate 상태";
        ...
        private void btn_Out_Click(object sender, EventArgs e)
        {
            ...
            {
                if (MessageBox.Show("총 " + view.SelectedRowsCount + "개의 Panel/Plate을 마감하시겠습니까?", "Delete", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    SplashScreenManager.ShowForm(_Main, typeof(frm_Wait), true, true, false);
        ...
        private void btn_Reuse_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("재사용할 Panel/Plate를 한 개 선택해주세요.", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }

            if (view.GetRowCellValue(view.GetSelectedRows()[0], "WAFER_STATUS_CODE").ToString() != "0005")
            {
                MessageBox.Show("Panel/Plate 상태가 실적등록인 Panel/Plate 재투입 가능합니다.", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            try
            {
                ...
                {
                    MessageBox.Show("Panel/Plate No : " + wafer_no + " 를 재투입 가능한 상태로 변경했습니다.", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception ex)
            {
