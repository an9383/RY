...
{
    public partial class frmESigManager_CMOS_OQC : DevExpress.XtraEditors.XtraForm
    {
        clsWork camWork = new clsWork();
        string query = "";

        public frmESigManager_CMOS_OQC()
        {
            InitializeComponent();
        ...
        {
            query = string.Format("exec CAMDBsh.RY_VM_Proc_CMOS_OQC_GetESigHistory '{0:yyyy-MM-dd}', '{1:yyyy-MM-dd}', N'{2}'",
                ...
                {   // 미검토
                    ...
                    {   //미승인 : Insert
                        query = string.Format("INSERT INTO IFRY.dbo.VM_ESigHistory(StepEntryTxnId, FactoryId, ContainerId, Gubun, WorkflowStepId, CheckUser, CheckDate, CheckComment, CheckUserName, TEMPLATE_FORM) "
                                            + "VALUES('{0}', '{1}', '{2}', 'OQC', '{3}', '{4}', GETDATE(), '{5}', '{6}', '{7}')"
        ...
        private void btnReport_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            string reportUrl = String.Format(@"{0}/View/QC/CMOS/Report_{1}.aspx?cmd=esig&reportid=&ContainerName={2}&StepEntryTxnId={3}&TEMPLATE_FORM={1}",
