...
{
    public partial class frmESigManager_TFT_Runsheet : DevExpress.XtraEditors.XtraForm
    {
        clsWork camWork = new clsWork();
        string query = "";

        public frmESigManager_TFT_Runsheet()
        {
            InitializeComponent();
        ...
        {
            query = string.Format("exec CAMDBsh.RY_VM_Proc_TFT_Runsheet_GetESigHistory '{0:yyyy-MM-dd}', '{1:yyyy-MM-dd}', N'{2}'",
        ...
        private void btnReport_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            if (drv["ITEM_TYPE"].ToString() == "SW")
            {
                reportUrl = String.Format(@"{0}/View/QC/TFT/Report_QC_TFT_SW_REPORT.aspx?cmd=esig&reportid=74&ContainerName={1}&StepEntryTxnId={2}",
                                WrGlobal.reportRootUrl, drv["ContainerName"].ToString(), drv["StepEntryTxnId"].ToString());
            }
            else
            {
                reportUrl = String.Format(@"{0}/View/QC/TFT/Report_QC_TFT_REPORT.aspx?cmd=esig&reportid=73&ContainerName={1}&StepEntryTxnId={2}",
                                WrGlobal.reportRootUrl, drv["ContainerName"].ToString(), drv["StepEntryTxnId"].ToString());
            }
